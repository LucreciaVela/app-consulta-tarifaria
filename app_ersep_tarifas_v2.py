...
if submitted:
    ...
    if origen_valido and destino_valido:
        ...
        if not resultados.empty:
            ...
            resultados = resultados[['Empresa', 'Modalidad', 'Tarifa']]
...
📍 Origen: {origen_valido.upper()}
📍 Destino: {destino_valido.upper()}
💲 Tarifa vigente: {fila['Tarifa']}
"""

            st.markdown("### Texto para compartir")
            st.text_area("Copiar el siguiente texto:", mensaje, height=100)

            col1, col2 = st.columns(2)
            with col1:
                st.markdown(f"<a href='https://wa.me/?text={quote(mensaje)}' target='_blank'><button style='background-color:#25D366;color:white;padding:8px;border:none;border-radius:5px;'>📲 Compartir por WhatsApp</button></a>", unsafe_allow_html=True)
            with col2:
                st.markdown(f"<a href='mailto:?subject=Tarifa Interurbano ERSeP&body={quote(mensaje)}'><button style='padding:8px;'>✉️ Compartir por Email</button></a>", unsafe_allow_html=True)
